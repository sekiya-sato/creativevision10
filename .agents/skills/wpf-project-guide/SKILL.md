...
description: Provides the shared CvWpfclient project conventions, resource usage, BaseWindow rules, validation flow, and build checks required before any WPF-related work.
---

# WPF Project Guide

このスキルは、`CvWpfclient` に関わるすべての作業で最初に参照する共通ガイドです。個別画面の新規作成や改修の前提となる、プロジェクト全体の構成・MVVM規約・リソース利用・検証方針を定義します。

## いつ使うか

- `CvWpfclient` 配下の `View` / `ViewModel` / `Helpers` / `Resources` / `MenuData.cs` を編集するとき
...
- View は主に `CvWpfclient/Views/` 配下
- ViewModel は主に `CvWpfclient/ViewModels/` 配下
- 共通リソースは `CvWpfclient/Resources/` 配下
- 共通UI補助は `CvWpfclient/Helpers/` 配下
- 画面起動導線は `CvWpfclient/Models/MenuData.cs` が関わる場合がある
...
- 業務画面やマスターメンテ画面では `Window` ではなく `CvWpfclient.Helpers.BaseWindow` を優先する
...
- 先に `CvWpfclient/Resources/UIColors.xaml`、`CvWpfclient/Resources/UICommon.xaml`、`CvWpfclient/Resources/UIMainWindow.xaml` を確認する
- 直接 `SolidColorBrush` を散在させるより、既存キーや共通スタイルを優先する
- WPFリソースや例外が絡む場合は、`CvWpfclient/App.xaml` と参照先ResourceDictionaryを先に読む
