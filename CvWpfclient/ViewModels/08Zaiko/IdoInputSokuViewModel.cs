...
protected override Type Tabletype => typeof(Tran05Ido);
...
protected override void OnCurrentEditChangedCore(Tran05Ido? oldValue, Tran05Ido newValue) {
	...
	async Task DoPrintList(CancellationToken ct) {
		...
		await RunPrintPdfAsync("IdoInputSoku_header.qfm", null, new QueryListSqlParam(typeof(Tran05Ido), BuildListPrintSql(query), query.Parameters), ct);
	}

	[RelayCommand(CanExecute = nameof(IsListTabSelected), IncludeCancelCommand = true)]
	async Task DoPrintDetail(CancellationToken ct) {
		var query = CreateListQueryParam();
		await RunPrintPdfAsync("IdoInputSoku_detail.qfm", null, new QueryListSqlParam(typeof(Tran05Ido), BuildDetailPrintSql(query), query.Parameters), ct);
	}
...
0 as Dummy2,
0 as Dummy3
from Tran05Ido {query.AddWhereOrder()}
";
	}

	static string BuildDetailPrintSql(QueryListParam query) {
		var denpyoSub = $"select * from Tran05Ido {query.AddWhereOrder()}";
