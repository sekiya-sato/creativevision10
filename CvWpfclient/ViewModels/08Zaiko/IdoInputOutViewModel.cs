...
public partial class IdoInputOutViewModel : Helpers.BasePlainLightMenteViewModel<Tran10IdoOut>, ITranInputTab {
...
protected override Type Tabletype => typeof(Tran10IdoOut);
...
protected override void OnCurrentEditChangedCore(Tran10IdoOut? oldValue, Tran10IdoOut newValue) {
	...
	[RelayCommand]
	void GoToDetail(Tran10IdoOut? item) {
	...
	async Task DoPrintList(CancellationToken ct) {
		...
		await RunPrintPdfAsync("IdoInputOut_header.qfm", null, new QueryListSqlParam(typeof(Tran10IdoOut), BuildListPrintSql(query), query.Parameters), ct);
	}

	[RelayCommand(CanExecute = nameof(IsListTabSelected), IncludeCancelCommand = true)]
	async Task DoPrintDetail(CancellationToken ct) {
		var query = CreateListQueryParam();
		await RunPrintPdfAsync("IdoInputOut_detail.qfm", null, new QueryListSqlParam(typeof(Tran10IdoOut), BuildDetailPrintSql(query), query.Parameters), ct);
	}
...
0 as Dummy2,
0 as Dummy3
from Tran10IdoOut {query.AddWhereOrder()}
";
	}

	static string BuildDetailPrintSql(QueryListParam query) {
		var denpyoSub = $"select * from Tran10IdoOut {query.AddWhereOrder()}";
...
";
	...
	void AddMeisai() {
		...
		var newMeisai = new Tran99Meisai { No = nextNo };
	...
	protected override string GetInsertConfirmMessage() => $"追加しますか？ (移動No={CurrentEdit.Id})";
	protected override string GetUpdateConfirmMessage() => $"修正しますか？ (移動No={CurrentEdit.Id})";
	protected override string GetDeleteConfirmMessage() => $"削除しますか？ (移動No={CurrentEdit.Id})";
}
