...
public partial class CoreService : ICoreService {
	...
	public CoreService(ILogger<CoreService> logger, IConfiguration configuration, IWebHostEnvironment env, IHttpContextAccessor httpContextAccessor,
		...
		_handlers = new Dictionary<CvFlag, Func<CvMsg, CallContext, CvMsg>> {
			...
			[CvFlag.Msg004_GetConnectionStatus] = HandleGetConnectionStatus,
			[CvFlag.Msg041_ConvertList] = HandlerGetConvertTaskList,
			...
			[CvFlag.Msg059_TranTaxRebuild] = (req, ctx) => HandleTranTaxRebuild(req, ctx),
		};
	}
