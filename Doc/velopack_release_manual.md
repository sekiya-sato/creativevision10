...
- `Cvnet10Wpfclient` を ClickOnce 相当の更新方式から Velopack 配布へ移行した後の、版数更新と配布手順をまとめる。
...
1. `Cvnet10Wpfclient/appsettings.json` の `Application.Version` を更新する
2. 必要に応じて `Cvnet10Wpfclient/appsettings.Production.json` の `Update:FeedUrl` を本番URLへ変更する
...
1. Windows のコマンドプロンプトで `Cvnet10Wpfclient` フォルダを開く
...
- publish 出力: `Cvnet10Wpfclient/bin/publish-velopack`
- Velopack の release 生成物: `vpk pack` の既定出力先

## 運用メモ

- `publish-velopack.bat` の末尾に `scp` コピー処理を追記できるよう `TODO` コメントを入れてある
