...
1. プログラムを更新して起動し、`UpdateDb` の `26_09_02_01` / `26_09_02_02` を適用する。
2. 期首日（`MasterSysman.FiscalStartDate`）以降の全期間について、次を再計算する。
   - 売掛残更新（`CalcSummaryUriKake`）
   - 買掛残更新（`CalcSummaryKaiKake`）
   - 請求計算（`CalcSummaryUriSei`）を全締日・全請求月について
   - 支払計算（`CalcSummaryKaiShi`）を全締日・全支払月について
3. 再計算しないと `Sonota` が 0 のままになる（`Balance` 自体は `26_09_02_02` の UPDATE で整合する）。
4. 売掛金管理表・買掛金管理表・請求一覧表・支払一覧表で、
   前月残＋当月増減＝当月残 が成立することを目視確認する。
...
**ステータス: 承認済み（2026-09-01）。Step 1〜8 実装完了。Step 9（最終確認・本番適用）は未実施。**
