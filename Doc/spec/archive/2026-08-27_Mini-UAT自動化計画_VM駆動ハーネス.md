...
状態: **Phase 0/1 実装・実施完了（2026-08-28）**。VM駆動ハーネス（`Doc/test/UatVm/`）を構築し、C-01〜C-10全ケースPASS。D-09（Mini-UATユーザー最終承認）は[結果レポート](../../test/UatVm_Phase01_20260828_テスト結果レポート.md)をもって完了済み。Phase 2以降（UAT-02/03/04通し、UAT-07原価境界、UAT-08移行）は`Doc/spec/2026-08-18_CV10機能完成度チェックリスト.md`側の残作業として継続管理する。
...
| ~~A-2~~ | ~~D-09の承認者としての最終サインオフ~~ | **2026-08-28 決定：合格とみなす**。Phase 0/1（[結果レポート](../../test/UatVm_Phase01_20260828_テスト結果レポート.md)）をもってD-09完了 |
| ~~A-3~~ | ~~D-01 原価4項目（最終仕入原価／総平均原価／諸掛／消化仕入）の10.0必須・10.1分類~~ | **2026-08-28 決定：原価4項目は10.1**。10.0は`MasterShohin.TankaGenka`（原価単価）をそのまま採用する。既存コードは`Gedai`（下代）へ`TankaGenka`を転記する経路が既にある（例: `CvServer/Services/PointOfSaleService.cs:160`）ため、10.0では新規実装不要 |
| ~~A-4~~ | ~~D-04 親子請求の出力方式、D-05 適格請求書の税率別内訳出力方式~~ | **2026-08-28 決定**：D-04は10.0で親子出力なし（単独請求のみ）。**D-05は税率別内訳が必要**、かつ**現状は未実装で改修が必須**（下記参照） |
...
| ~~A-6~~ | ~~在庫／原価の期首残の投入元（D-08残件）~~ | **2026-08-28 決定：期首の棚卸入力データを採用**。期首日が2026/04/01なら2026/03/31時点の棚卸入力データを期首在庫・期首原価として使う。専用画面や外部CSVは不要、既存の棚卸機能に乗せる方針 |
| ~~A-7~~ | ~~D-06 総平均原価の`TQ`の解釈~~ | **2026-08-28 決定：`TQ`はcv10では無視する**。旧cvnetでは調整金額の意味だったが、cv10の総平均原価計算では扱わない |
...
## 6.4 決定：D-05適格請求書の税率別内訳は現状未実装、改修が必須（2026-08-28）

A-4の判断材料として、請求書（`SeikyuBalanceDetailView`/`ViewModel`）・帳票フォーム
（`printform/SeikyuBalanceDetail.qfm`）・`SummaryUriSei`スキーマを調査した。

### 現状（すべて調査済み、未実装であることを確認）

- `SeikyuBalanceDetailViewModel.cs`のSQLは`SummaryUriSei.Tax`（標準・軽減の合算1本）を
  そのまま出力するのみで、税率別の計算式は存在しない。
- `SeikyuBalanceDetail.qfm`はitem1〜15の固定項目で、税率別内訳を印字するフィールドが無い
  （消費税はitem8に単一合算値として印字）。
- `SummaryUriSei`（`CvBase/BaseDbKake.cs:117-257`）の税額列は`Tax`1本のみで、標準/軽減を
  分ける列は存在しない。
- `Doc/spec/archive/2026-08-25_明細別消費税計算_詳細設計.md`4.8節が既にこの課題を認識しており、
  「案1: `SummaryUriSei`へ税率別列を追加」「案2: 出力時に`Tran*`明細を直接集計」の
  二択を提示した上で「どちらを採るかはD-05で決定する」として未決のまま残していた。

### 決定（2026-08-28、ユーザー判断）

D-05は10.0の必須要件とする（税率別内訳が必要）。ただし方式（案1/案2のどちら）は
未決定のまま残る。改修が必要な箇所は次の3点：

1. 保存方式の決定：`SummaryUriSei`へ税率別列（または内訳JSON）を追加するか、
   印刷時に明細（`Id_Tax`/`TaxRate`/`Tax`、2026-08-25実装済み）を直接集計するか。
   **2026-08-28 ユーザーコメント**：Summary系テーブルに税率別Taxを持たせるか、
   明細から集計するロジックが必要になるだろうとの見立て（＝案1・案2いずれかの
   実装が必須という認識で一致。方式そのものはまだ未決）。
2. 集計ロジックの新規実装：`SummaryDb.cs`（請求計算、案1の場合）または
   `SeikyuBalanceDetailViewModel`のSQL（案2の場合）。
3. `SeikyuBalanceDetail.qfm`への税率別課税標準額・税額を印字するitem/フィールド追加。

10.0 Release Gate（帳票突合）の前提条件であり、着手順としてはD-01/D-06/D-08と並行できる。

### `Tran02Material`区分99（消費税調整）の税率別内訳への含め方（2026-08-28 決定）

C-09（生地・付属仕入の合算）で確認したとおり、`Tran02Material`区分99（その他）の金額は
仕入へ畳み込まず消費税へ全額計上する（`CvDomainLogic/SummaryDb.cs`のコメント参照）。
紐づく資材は消費税調整用プレースホルダ（`MasterMaterial.Code=000099`「消費税」）で、
`Id_Tax=0`（非課税）として登録されており、税率と紐づける設計にはなっていない。

**決定：一律10%（標準税率）として税率別内訳へ含める。** 生地・付属仕入は軽減税率(8%)対象に
なり得ないため、実務上ほぼ全て標準税率相当という前提に基づく。仕入側（支払明細）の
税率別内訳自体がD-05のスコープに入るかは別途D-05の適用範囲確定時に扱う。
