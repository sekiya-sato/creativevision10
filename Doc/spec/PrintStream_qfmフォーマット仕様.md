...
1. **XML 文法（タグ・属性・出現構造・値の分布）= 実 qfm コーパスの機械マイニング**
   - `C:\gitroot\new2022\cv10\printform\*.qfm` … 129 本（cv10 実運用）
   - `C:\gitroot\cv\cvnet_pkg\cvnetpss\*.qfm` … 1968 本（旧 cv.net 資産、サブフォルダ含む）
   - 合計 **2097 本**。抽出は `Doc/spec/tools/extract_qfm_grammar.ps1` で再現可能。
...
- 文字コードは **Shift_JIS(cp932)**、改行は **LF**。XML 宣言は `encoding="SHIFT_JIS"`（または `shift_jis`）。
- ルート要素は `<printstream version="3.0">`（コーパス 2097/2097 が `3.0`）。 `[M]`
- データ入力は **CSV のみ**。コーパスの `<path datatype="...">` は 2094/2094 が `csv`。固定長・XML 入力は実運用で使わないため本仕様では扱わない。 `[M]`
...
コーパス全体の要素出現数（2097 ファイル）: `[M]`

| 要素 | 出現数 | 役割 |
|---|---:|---|
| `printstream` | 2097 | ルート |
| `datadesc` | 2097 | データ定義ブロック |
| `datarecord` | 2094 | 論理項目(item)定義 |
| `item` | 122075 | 論理項目 1 個 |
| `prefix` | 869 | CSV レコード種別プレフィックス（複数レコード様式） |
| `file` | 2094 | 入力ファイル指定 |
| `path` | 2094 | 入力ファイルパス／種別 |
| `page` | 2099 | 用紙・ページ |
| `region` | 2519 | レコード印刷領域 |
| `record` | 7169 | レイアウト単位（明細・見出し・中計など） |
| `group` | 3392 | 集計・改ページグループ |
| `text` | 136173 | テキストフィールド |
| `image` | 244 | イメージフィールド |
| `barcode` | 209 | バーコードフィールド |
| `data` | 134412 | フィールドのデータソース設定 |
| `decode` | 136382 | フィールドの書式（属性＋編集文字列） |
| `font` | 129505 | フォント設定 |
| `color` | 146314 | 色・透過設定 |
| `border` | 79781 | 罫線 |
| `position` | 225716 | 位置・大きさ（ほぼ全フィールド／領域） |
| `cceffect` | 516 | 網掛け・交互色などの効果 |
| `cc` | 101 | カラーコード指定 |
| `script` | 712 | スクリプト |
