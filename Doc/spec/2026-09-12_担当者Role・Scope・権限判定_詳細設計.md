...
状態: **10.0基盤実装済み・10.2判定処理未実装**。本書は [`Doc/spec/2026-09-05_CV10機能完成度チェックリスト.md`](2026-09-05_CV10機能完成度チェックリスト.md) の `D-10`（権限判定・メニュー公開状態）への回答。10.0でテーブル定義・基盤を実装し、10.2以降で判定処理を有効化する（0.3節）。
...
CV10の`*View.xaml`（218件）を接尾辞で数えると次のとおりで、対象は`Mente`19件＋`Input`20件＝**39画面**、残り179画面は対象外である。
