...
| C-05 冪等性 | 画面 | 画面から2回実行し、請求番号を維持して値が一致（計算層は済、画面経路で再確認） |
| C-06 明示的再発行 | 画面 | `IsReissue=true`で`Renban+1`。`SupportsReissue`が偽の画面では無効化されること |
| C-07 Rebuild締日変更ブロック | 画面 | 締日変更検出時に全要求を送信せず、画面へ理由が提示される（D-02） |
| C-08 入力検証 | 画面 | 月形式不正・締日未選択・コード範囲逆転で、送信せず警告ダイアログのみ出る |
| C-09 生地・付属仕入の合算 | 計算＋画面 | `Tran02Material`の買掛合算と区分99（その他）の消費税全額計上（UAT-06残作業） |
| C-10 キャンセル | 画面 | `ExecuteCancelCommand`で中断し、DBが中途半端な状態にならない |
