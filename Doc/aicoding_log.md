...
### 目的
- サーバの自動実行ジョブ（スケジューラ）に、クライアントから実行有無を切り替えられるフラグを追加する。普段は実行せず、必要なときだけスケジュール設定して実行させられるようにする。あわせて `SysExecMiscView` の重い保守処理3件を自動実行ジョブへ追加する。

### 実施内容
- `ISchedulerService` に `SchedulerTaskType.MasterShohinMeishoRebuild=3`/`MasterVColumnResync=4`/`TranTaxRebuild=5` と、`SchedulerTaskInfo.IsEnabled`/`CheckMinInterval`/`MinIntervalMinutes`、`SetSchedulerTaskEnabledRequest`、`SetTaskEnabledAsync` を追加。
- `MasterConfig`（`CvBase/BaseDb1Master.cs`）に `CategorySystem`/`CategoryScheduler`/`NameJodaiKeepDays`/`NameSchedulerJobPrefix`/`NameSchedulerEnabledSuffix`/`NameSchedulerCronSuffix` の定数を追加し、実行フラグとcron式を `MasterConfig`（Category=`Scheduler`）へ永続化する。レコード無しは未設定＝ジョブ定義の既定値を使う。
- `CvDomainLogic/SchedulerJobConfigDb.cs`（新規）で `GetEnabled`/`GetCron`/`SetEnabled`/`SetCron` を実装。
- `CvServer/Services/SchedulerService.cs` に `SchedulerJobDefinition` レコードと7件の `SystemJobDefinitions`、`CalculateMinIntervalMinutes` による起動間隔算出、`SetTaskEnabledAsync`、新規3ジョブ（商品名称再構築／V*列再同期／伝票税額再更新）の登録処理を追加。3ジョブは重い処理のため既定で実行フラグOFF、起動間隔1時間未満はエラーとする（内部フラグ `SchedulerService.MinIntervalCheckEnabled` で有効/無効を切替可能）。`CvServer/Program.cs` で起動時登録。
- `CvWpfclient` の `SysSchedulerJobMenteView(Model)` に「実行」列と「実行する/しない 切替」ボタン（確認ダイアログ付き）を追加。

### テスト追加
- `Tests/TestServer/TestServer.cs` の既存 `RegisterDailySqliteWalCheckpointTask_RegistersTwoAmSchedule` 等に倣い、DB・サーバ起動不要な純粋ロジックのテストを5件追加した。
  - `CalculateMinIntervalMinutes_ReturnsMinimumIntervalMinutesForVariousCronExpressions`: 毎日1回=1440分、毎時=60分、30分毎=30分、既存ワークファイル削除cron(`30 0,12 * * *`)=720分を検証。
  - `SystemJobDefinitions_HasSevenUniqueEntries`: 定義が7件、`TaskId`/`JobKey`が全件ユニークであることを検証。
  - `SystemJobDefinitions_NewHeavyJobsAreDefaultDisabledWithMinIntervalCheck`: 新規3ジョブが `DefaultEnabled=false` かつ `CheckMinInterval=true` であることを検証。
  - `SystemJobDefinitions_ExistingJobsAreDefaultEnabledWithoutMinIntervalCheck`: 既存4ジョブが `DefaultEnabled=true` かつ `CheckMinInterval=false` であることを検証。
  - `SystemJobDefinitions_CheckMinIntervalJobsSatisfyMinimumIntervalThreshold`: `CheckMinInterval=true` の全ジョブについて、既定cron式の最小発生間隔が `SchedulerService.MinIntervalMinutes`（60分）以上（または算出不能=null）であることを検証。今回要件の要となるテスト。
  - 既存テストは変更していない。`using NCrontab;`/`using System.Linq;` を追加。

### 検証（Step5-2）
- `C:\gitroot\UT\vscmd.bat dotnet build creativevision10.slnx`: 成功（警告0、エラー0）。
- `C:\gitroot\UT\vscmd.bat dotnet test Tests\TestServer\TestServer.csproj --filter FullyQualifiedName~CalculateMinIntervalMinutes`: 1件成功。
- 同 `--filter FullyQualifiedName~SystemJobDefinitions`: 4件成功。
- 同 `--filter FullyQualifiedName~Register`（既存2件を含む回帰確認）: 3件成功、既存挙動に影響なし。
- `git diff --check`: 空白エラーなし（クリーン）。

### 変更ファイル
- `Tests/TestServer/TestServer.cs`（テスト5件追加、using 2件追加）
...
## [2026-09-02] 店舗予算表(ShopBudgetReportView)を旧CVnet仕様で再実装

### Agent
- Sekiya Sato Claude Opus 5 Middle

### 目的
- 旧CVnetの「予算 : 予算表」(program_crs / isqlqfw00.aspx の `_30_yosan` / cvnet30prn_yosan.qfm) に合わせて、cv10 の店舗予算表を再実装する。参照資料は `refer/printwrk/ShopBudgetReportView-sample/`。

### 実施内容
- `printform/ShopBudgetReport.qfm` を旧 `cvnet30prn_yosan.qfm` のコピーへ差し替えた。変更点は `<page>` の `id` を cv10 慣例の `Form1` にした1点のみで、レイアウト・item定義・decode書式は旧フォームのまま。A4横・item1〜item27。cv10の他フォームは全て `id="Form1"`。
- `ShopBudgetReportViewModel` のSQLを旧 `_30_yosan` の組み立てどおりに書き換えた。旧システムがワークテーブル `HC$WKS_HAN01` へ器を作って UPDATE を重ねていた処理を、CTE (calendar → shops → budget/tenuri/sales → han01 → cum) で再現している。出力27列は qfm の item1〜item27 と1:1で、列順と列数は変更禁止。
- 出力対象を「予算を組んだ店舗のみ」へ変更した。旧は `HC$MASTER_YO_TENPO` を店舗でGROUP BYした集合が出力の起点で、画面の赤字注意書きもそう書かれている。従来のcv10実装は全直営店を起点にしていた。
- 売上の符号と区分を旧 `取引区分` の規則へ合わせた。旧は `DECODE(TRUNC(取引区分/10),1,1,2,-1,0)` で10番台=売上/20番台=返品の符号を付け、`MOD(取引区分,10)` で 0=プロパー / 1=セール / 4=社販 を分けていた。cv10 は `Tran01Tenuri.Kubun` が同じ体系(10/11/20/21)で、返品も `KingakuTotal` が正値のため符号付けが必要。従来のcv10実装は `SUM(KingakuTotal)` で返品を加算していた。
- 整数除算のDB間意味差を避けるため、符号判定は `Kubun / 10` ではなく `Kubun BETWEEN 10 AND 19` / `BETWEEN 20 AND 29` の範囲比較で書いた。
- 前年日付は旧仕様の2方式を実装した。日付対比は前年同月同日（閏年2/29は旧 `IS_DATE` 判定で対象外になるため前年売上・前年累計を0）、曜日対比は `NEXT_DAY(ADD_ADJMONTH(日付,-12), 日付の曜日)`。#32839 対応（前年が閏年で前年日付が2/28のとき前年2/29分を2/28へ含める）も入れた。Oracle関数依存の分岐をSQLiteで書くと壊れやすいため、日付側はC#で確定させてSQLへ VALUES として渡している。
- `ShopBudgetReportView.xaml` から旧画面に無い「出力対象(全て/当年売上あり)」ラジオを削除し、注意書きを旧画面の赤字文言に合わせた。残る選択肢は旧画面と同じ「出力区分(店舗別/全店)」と「前年比(日付対比/曜日対比)」。

### 確認
- 印刷エンジンの日付解釈を実測した。qfm の `datatype="date"` 項目は `yyyyMMdd` を日付として読まず空白印字になる。`yyyy/MM/dd` / `yyyy/MM/dd HH:mm:ss` / `yyyy-MM-dd` は解釈される。よってSQLは `yyyy/MM/dd` で出し、qfm の decode 書式(`YY"年"MM"月"` と `DD`)は旧フォームのまま使う。
- 27列のCSVを実際に PrintStream へ通してPDFを生成し、内容を検証した。2店舗×3日で2ページ（item3=店舗CD の level1 pagechange が効く）、店舗名・`(合計)`・`26年06月`・グループ合計の `予算 321（千円）`・`前年日付 2025/06/01(日) ～ 2025/06/30(月)`・明細16列が旧レイアウトどおり出力された。
- 売上区分の集計を旧システムと突き合わせた。2022/05の6店舗(000016/000029/000031/000033/000034/000037)で SU01(売上)・SU03(プロパー)・SU04(セール)・SU05(総売上)・SU20(売上点数) が Oracle と全件一致。
- 日付対比の前年比を旧システムと突き合わせた。店舗000016・2022/05の31日分で 売上・前年売上・累計・前年累計・前年比(TRUNC 1桁: 226.8 / 845.6 / 69 / 98 など) が Oracle と全件一致。
- 曜日対比の前年日付を旧Oracle式と突き合わせた。2026/06全30日、2024/02全29日(閏年)、2025/03/01・2025/02/28・2023/01/01・2023/12/31 の計63件すべて一致。`ADD_ADJMONTH` は `ADD_MONTHS` と同じ月末クランプ挙動だった。
- `CvWpfclient` ビルド成功。`Tests/TestSqlDialect` は135件全て成功（符号判定を範囲比較にする前は「整数除算を含むSQLの本数」の上限13本を超えて失敗していた）。`git diff --check` クリーン、qfm は cp932・CRLF 維持。
- 未対応（cv10に対応するテーブル・列がないため空欄/0固定。ユーザー了承済み）: 社販売上（`Kubun` に社販がない）、客数（旧 `HC$TRAN_KYAKU` 相当の `Tran04PosSeisan.KyakuSu` が未運用）、既存比（`MasterTokui` に開店日相当が無い）、商品外除外（旧 `商品区分FLG=0` の明細絞り込み。cv10 は売上明細がJSON列で集計に使えない）。
- 実機PDFの確認条件: `server-user163.db` で予算データがあるのは 2026/06 のみのため、年月に `2026/06` を指定した場合だけ行が出る（対象店舗は 000014 富山 / 000028 イオンモール盛岡）。この2店舗は当年も前年も売上実績が無いため、売上・前年比・前年累計は0で、予算・予算累計・予算差異のみ値が入る。売上まで入ったPDFを見るには、売上実績のある月（例 2022/01。前年 2021/01 も実績あり）へ店舗ブランド予算マスタで予算を登録してから実行する。
...
### 目的
- 月次請求書（請求書印刷）を適格請求書の記載要件へ合わせる。登録番号の印字と、軽減税率(Id_Tax=2)の`*`印・税率表示。

### 実施内容
- `MasterSysman.TaxRegistrationNumber`を`item33`（旧・自社FAX欄）へ出力した。qfmのラベル`FAX:`→`登録番号:`と欄幅拡張は人手で実施。
- 軽減税率の判定を`Id_Tax = 2`とした。明細単位は明細JSON(`Jmeisai`)の行ごとの`Id_Tax`スナップショットで判定し、商品名へ`*`を付けて税率も行ごとに表示する。伝票単位は1行=1伝票のため、伝票内に軽減税率明細を含めば取引区分名へ`*`を付け、税率欄には使用税率を併記する（`10%/8%`）。
- qfmの静的バインドを機械抽出し、CSV列マッピングを実バインドへ合わせて全面的に修正した。旧実装は旧CRSの列名一覧`d_sql.txt`を根拠にしていたが、qfmの実際の用途と一致しておらず、商品名・住所・繰越金額が別欄へ流れ込み、明細単位では商品CDが金額欄へ数値解釈されていた。
- `item56`〜`60`は伝票単位（税率別内訳）と明細単位（商品明細行）で用途が異なるため`IsMeisaiUnit`で分岐した。
- 旧CRS由来の「CSVは常に61列」制約を撤廃し、明細単位の税率別内訳（`item62`〜`65`）と画像（`item66`）のため66列へ拡張した。
- `SumForRate`を共通メソッド化し、`headers` CTEへ`Tax1/2/3`・`TaxableAmount1/2/3`を追加して税率別内訳の集計式を印刷SQLからも使えるようにした。

### 確認
- `dotnet build creativevision10.slnx -v q --nologo`が警告0・エラー0。
- 実データ（得意先100590 / 請求日2022-04-30）を`cv-sqlite`で直接実行し、課税対象額20,130(10%)+735(8%)=20,865=当月御買上額、消費税0+59=59、合計20,924を確認した。
- `tools/qfmprint`で両様式をPDF描画し、登録番号・税率別内訳・明細ごとの税率(10%/8%/8%)・`*`印の位置・伝票単位の`10%/8%`併記を確認した。
